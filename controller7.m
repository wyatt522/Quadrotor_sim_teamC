...
classdef controller7 < handle
    properties(Access = public)
        zdes(12,1) double;
        K_position(4,12) double;
        K_velocity(4,12) double;
    ...
    methods(Access = public)
        function obj = controller7(quadrotor)
            ...
            Q = diag([10 10 10 1 1 1 0.1 0.1 0.1 1 1 1]);
            R = 2*eye(4);
        ...
        function u = output(obj, ~, z, y)
            ...
            switch_radius = 1;
            if iscaptured == false && (norm(y(1:3) - z(1:3)) > switch_radius)
                [ref_pos, ref_vel] = obj.calc_ref(z, y);
                obj.zdes(1:3) = ref_pos;
                obj.zdes(7:9) = ref_vel;
                u = obj.u0 + obj.K_velocity * (obj.zdes - z);
                disp("VELOCITY")
            elseif iscaptured == false && (norm(y(1:3) - z(1:3)) <= switch_radius)
                [ref_pos, ref_vel] = obj.calc_ref(z, y);
                obj.zdes(1:3) = ref_pos;
                obj.zdes(7:9) = ref_vel;
                u = obj.u0 + obj.K_position * (obj.zdes - z);
                disp("POSITION")
